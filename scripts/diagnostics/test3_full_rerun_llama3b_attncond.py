#!/usr/bin/env python3
"""Test 3 full rerun — checkpoint_best.pt, Llama-3.2-3B-Instruct +
attention-conditioned aggregator (job 66527933, completed 2026-07-28, best
val_loss=0.4413). Same post-fix InterSliceAggregator-family architecture
as test3_full_rerun_llama3b_v2.py, but conditioning="attention" (replaces
FiLM entirely with a second cross-attention operation over the instruction
embedding — see aadp/ablations/attention_conditioned_ctclip_stage2.py).
...
CKPT_PATH = "/scratch/sadeniji/ictc_checkpoints_llama3b_attention_cond/checkpoint_best.pt"
CFG_PATH = "configs/ctclip_stage2_llama3b_attention_cond.yaml"
...
def main():
    ...
    logger.info("SUMMARY")
    logger.info("=" * 80)
    logger.info("Part A cosine: mean=%.6f min=%.6f max=%.6f  (BioMedLM+attention-cond=0.8390; "
                 "Llama+FiLM=see test3_full_rerun_llama3b_v2.py)", cos_a["mean"], cos_a["min"], cos_a["max"])
