#!/usr/bin/env python3
"""Test 3 full rerun — checkpoint_best.pt, Llama-3.2-3B-Instruct + bounded
V-FiLM fix (job 66667523, completed 2026-07-30, best val_loss=0.3995) —
warm-started from the pre-fix checkpoint (job 66527771, val_loss=0.4225)
via configs/ctclip_vfilm_fix.yaml's warm_start_drop_keys, with beta_v_proj
removed entirely and gamma_v bounded to [0.7, 1.3] via tanh (see
aadp/models/projector/stage2.py).

This is the direct comparison point for scripts/check_vfilm_scan_diversity.py's
finding (job 66661482) that the OLD, unbounded gamma+beta V-FiLM formula
collapsed cross-scan direction (V_after cosine 0.999 across every scan-pair
group under a fixed instruction). Same protocol as every prior Test 3 rerun
this session for direct comparability.
...
CKPT_PATH = "/scratch/sadeniji/ictc_checkpoints_vfilm_fix/checkpoint_best.pt"
CFG_PATH = "configs/ctclip_vfilm_fix.yaml"
...
def main():
    ...
    logger.info("SUMMARY")
    logger.info("=" * 80)
    logger.info("Part A cosine: mean=%.6f min=%.6f max=%.6f  (pre-fix Llama+FiLM=see docs/DIAGNOSTIC_REPORT.md)",
