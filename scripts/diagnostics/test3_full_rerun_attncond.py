#!/usr/bin/env python3
"""Test 3 full rerun — checkpoint_best.pt, attention-conditioned ablation,
post 3-epoch full training.

Read-only diagnostic. Loads the trained checkpoint at
/scratch/sadeniji/ictc_checkpoints_stage2_attention_cond/checkpoint_best.pt
(step=3000, val_loss=0.8795) and runs:
  Part A — instruction sensitivity: same scan, 5 instructions
  Part B — scan sensitivity: same instruction, 10 scans (PRIMARY TEST)

Unlike prior Test 3 reruns this session (CPU, synthetic random features,
projector-only forward pass), this does REAL text generation through the
full BioMedLM LLM on REAL validation scans, on GPU.
"""
import logging
import re
...
CKPT_PATH = "/scratch/sadeniji/ictc_checkpoints_stage2_attention_cond/checkpoint_best.pt"
CFG_PATH = "configs/ctclip_stage2_attention_cond.yaml"
...
def load_scan_list():
    """Filtered, CSV-order list of (stem, pt_path) for every valid row with
    a matching .pt file — mirrors the indexing convention used by every
    prior Test 3 rerun this session."""
...
def cosine_stats(vectors):
    """vectors: (N, C). Returns pairwise cosine stats over all N*(N-1)/2 pairs."""
...
def edit_distance(a: str, b: str) -> int:
    """Classic O(len(a)*len(b)) Levenshtein DP, character-level."""
    la, lb = len(a), len(b)
    if la == 0:
        return lb
    if lb == 0:
...
def generate_report(model, features_1, instruction: str) -> str:
    """Replicates CTCLIPStage2VLM.forward()'s inference path exactly, except
    max_new_tokens=200 (spec) instead of the model's hardcoded 256 — forward()
    itself is not modified, this just calls the same public sub-components
    (instruction_encoder, projector, visual_proj, tokenizer, llm) directly."""
...
def main():
    ...
    logger.info("Building CTCLIPStage2VLM (conditioning=%s, llm=%s) on %s...",
                cfg.get("aggregator_conditioning", "film"), cfg.get("llm_model_name"), DEVICE)
    ...
    collapse_rate = len(near_dup_pairs) / total_pairs  # near_dup_pairs is a superset of exact_dup_pairs (edit_dist=0 < 0.05)
    ...
    logger.info("SUMMARY")
    logger.info("=" * 80)
    logger.info("Part A cosine: mean=%.6f min=%.6f max=%.6f  (history: pre-fix=0.9999, cosine-attn-fix=0.9928)",
                 cos_a["mean"], cos_a["min"], cos_a["max"])
    logger.info("Part B cosine: mean=%.6f min=%.6f max=%.6f  (target: meaningfully < 0.90)",
                 cos_b["mean"], cos_b["min"], cos_b["max"])
    logger.info("Part B collapse rate: %.1f%% -> %s", collapse_rate * 100, verdict_band)
