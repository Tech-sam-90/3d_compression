...
post 5-epoch full training (jobs 66274952 smoke, 66275278 full train,
both completed 2026-07-23, predating this session's collapse-diagnostic
work — never previously tested against Test 3).

Read-only diagnostic. Loads
/scratch/sadeniji/ictc_checkpoints_llama3b/checkpoint_best.pt
(step=4500, epoch=3, val_loss=0.5528) and runs:
  Part A — instruction sensitivity: same scan, 5 instructions
  Part B — scan sensitivity: same instruction, 10 scans (PRIMARY TEST)

Uses configs/ctclip_stage2_llama3b.yaml as-is: llm_model_name AND
instruction_encoder_model are both meta-llama/Llama-3.2-3B-Instruct
(no cond_dim projection layer exists, so they must match), conditioning
defaults to "film" (config never sets aggregator_conditioning) — this is
the FiLM aggregator, NOT the attention-conditioned ablation.

Same SCAN_INDICES / INSTRUCTIONS_5 / FIXED_INSTRUCTION as
scripts/diagnostics/test3_full_rerun_attncond.py so results are directly
...
CKPT_PATH = "/scratch/sadeniji/ictc_checkpoints_llama3b/checkpoint_best.pt"
...
def generate_report(model, features_1, instruction: str) -> str:
    ...
    max_new_tokens=200 instead of the model's hardcoded 256 — forward()
...
def main():
    ...
    logger.info("SUMMARY")
    logger.info("=" * 80)
    logger.info("Part A cosine: mean=%.6f min=%.6f max=%.6f  (BioMedLM+FiLM history: pre-fix=0.9999, cosine-attn-fix=0.9928; "
