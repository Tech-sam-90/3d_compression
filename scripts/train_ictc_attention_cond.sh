...
#SBATCH --job-name=ictc_train_attncond
#SBATCH --output=/scratch/sadeniji/logs/attncond_%j.out
#SBATCH --error=/scratch/sadeniji/logs/attncond_%j.err

mkdir -p /scratch/sadeniji/logs
mkdir -p /scratch/sadeniji/ictc_checkpoints_stage2_attention_cond
...
cd ~/3d_compression
python scripts/train_ctclip.py --config configs/ctclip_stage2_attention_cond.yaml
